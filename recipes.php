...
if($keyword) {
	$query = "SELECT * FROM recipes WHERE steps like '%$keyword%'";
	if($email) {
		$querylog = "INSERT INTO searchLog (keyword, username) VALUES ('$keyword','$email')";
		mysqli_query($link, $querylog);
	}
}
elseif($tag) {
	$query = "SELECT * FROM recipes NATURAL JOIN recipe_tags WHERE tagname='$tag'";
	if ($email) {
		$querylog = "INSERT INTO searchLog (keyword, username) VALUES ('$tag','$email')";
		mysqli_query($link, $querylog);
	}
}
else {
			...
			<div class="row">
				...
				<section class="content three-fourth">
					...
					?>
						<?php 
							$rid = $row['rid'];
							$href = "href=\"recipe.php?rid=";
							$quotation = "\"";
							$combined = $href . $rid . $quotation;
						?>
							<div class="entry one-third">
								<figure>
									<img src="images/cooking-college.jpg"/>
									<figcaption><a <?php echo $combined?>><i class="icon icon-themeenergy_eye2"></i> <span>View recipe</span></a></figcaption>
								</figure>
								<div class="container">
									<h2><a href="recipe.php"><?php echo $row['title']?></a></h2> 
								</div>
							</div>
					<?php	
						}
